...
export default function AdsenseModal({ apiKey, onClose }) {
  ...
  useEffect(() => {
    if (!show) onClose?.();
  }, [show, onClose]); // onClose কে ডিপেন্ডেন্সি অ্যারে যোগ করা হয়েছে
      ...
      <div className="bg-white rounded-xl shadow-2xl p-8 max-w-sm w-full text-center relative">
        ...
        <div className="my-4">
          <span className="text-gray-500 text-sm">
            Google Adsense (API Key: {apiKey || "demo"})
          </span>
        ...
        <button 
          className="mt-4 bg-green-500 text-white px-6 py-2 rounded-full font-bold hover:bg-green-600" 
          onClick={() => setShow(false)}
        >
          বন্ধ করুন
        </button>
